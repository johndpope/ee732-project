...
resutdir = './results_dbmcmc';
mkdir(resutdir);

nSubject = length(interconn.disleksi_dbmcmc_s2_prestim);
nNode = length(node_names_merged);

interstruct_merged = MergeNodes(merge_nodes, interconn.disleksi_dbmcmc_s2_prestim);
inter_disleksi_pre = reshape(cell2mat(interstruct_merged), [nNode nNode nSubject]);

interstruct_merged = MergeNodes(merge_nodes, interconn.disleksi_dbmcmc_s2_poststim);
inter_disleksi_post = reshape(cell2mat(interstruct_merged), [nNode nNode nSubject]);

interstruct_merged = MergeNodes(merge_nodes, interconn.kontrol_dbmcmc_s2_prestim);
inter_kontrol_pre = reshape(cell2mat(interstruct_merged), [nNode nNode nSubject]);

interstruct_merged = MergeNodes(merge_nodes, interconn.kontrol_dbmcmc_s2_poststim);
...
fig1 = figure('color', 'white', 'color', 'white', 'units', 'normalized', 'outerposition', [0 0 1 3/4]);
